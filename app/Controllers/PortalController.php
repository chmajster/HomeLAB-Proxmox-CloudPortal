...
final class PortalController
{
    ...
    public function page(Request $request): Response
    {
        ...
        $allowed = ['dashboard', 'vms', 'create-vm', 'projects', 'networks', 'templates', 'cloud-init', 'ssh-keys', 'ansible', 'activity', 'security', 'users', 'infrastructure', 'proxmox', 'firewall', 'storages', 'plans', 'blueprints', 'quotas', 'audit', 'settings'];
        if (!in_array($page, $allowed, true)) {
            return Response::redirect($this->app->url('/dashboard'));
        }
        $adminOnly = ['users', 'infrastructure', 'proxmox', 'firewall', 'storages', 'plans', 'blueprints', 'quotas', 'audit', 'settings'];
