...
final class OpenApiController
{
    ...
    public function spec(Request $request): Response
    {
        ...
        return Response::json([
            ...
            'info' => [
                ...
                'description' => 'REST API portalu Self-Service IaaS dla Proxmox VE. Operacje mutujące wymagają aktywnej sesji oraz nagłówka X-CSRF-Token.',
            ],
            'servers' => [['url' => $this->app->basePath() === '' ? '/' : $this->app->basePath(), 'description' => 'Current installation']],
            'tags' => array_map(static fn (string $name): array => ['name' => $name], ['System','Auth','Catalog','VM','Jobs','Cloud-Init','SSH','Audit','Administration']),
            'paths' => $paths,
    ...
    private function routes(): array
    {
        return [
            ...
            ['GET','/api/v1/admin/system/health','Administration','Administrative health status'], ['POST','/api/v1/admin/jobs/{jobId}/retry','Administration','Retry failed job'],
    ...
    private function decorate(array &$paths): void
    {
        ...
        $paths['/api/v1/admin/audit/search']['get']['parameters'] = $this->auditParameters();
        $paths['/api/v1/admin/audit/export']['get']['parameters'] = [...$this->auditParameters(), ['name' => 'format','in' => 'query','schema' => ['type' => 'string','enum' => ['csv','json']]]];
    ...
    private function components(): array
    {
        return [
            ...
            'schemas' => [
                ...
                'AuditEntry' => ['type' => 'object','properties' => ['id' => ['type' => 'integer'], 'created_at' => ['type' => 'string'], 'username' => ['type' => ['string','null']], 'ip_address' => ['type' => 'string'], 'action' => ['type' => 'string'], 'result' => ['type' => 'string','enum' => ['success','failure']], 'project_id' => ['type' => ['integer','null']], 'project_name' => ['type' => ['string','null']], 'virtual_machine_id' => ['type' => ['integer','null']], 'vm_name' => ['type' => ['string','null']], 'job_id' => ['type' => ['integer','null']], 'job_public_id' => ['type' => ['string','null']], 'proxmox_upid' => ['type' => ['string','null']], 'metadata' => ['type' => ['object','null']]]],
    ...
    private function pathParameters(string $path): array
    {
        ...
        return array_map(static fn (string $name): array => ['name' => $name,'in' => 'path','required' => true,'schema' => ['type' => in_array($name, ['node','device','action','snapshotName','type','resource'], true) ? 'string' : 'string']], $matches[1] ?? []);
    }
