...
final class AdminController
{
    ...
    public function create(Request $request): Response
    {
        ...
        $id = match ($resource) {
            ...
            'quotas' => $this->upsertQuota($request),
    ...
    public function update(Request $request): Response
    {
        ...
        $id = (int) $request->param('id');
        if ($id <= 0) {
            throw new HttpException(422, 'Invalid resource ID.');
        }
        match ($resource) {
    ...
    public function syncProxmox(Request $request): Response
    {
        $this->admin();
        $this->app->csrf->verify($request);
        $id = (int) $request->param('id');
    ...
    {
        $this->admin();
        $discovery = new ProxmoxNetworkDiscovery(
    ...
    {
        $this->admin();
        $discovery = new ProxmoxCatalogDiscovery(
            new ProxmoxClientFactory($this->app->pdo(), $this->app->crypto()),
        );
    ...
    {
        $this->admin();
        $inventory = (new ProxmoxVmDiscovery(
            new ProxmoxClientFactory($this->app->pdo(), $this->app->crypto()),
    ...
    public function projectMembership(Request $request): Response
    {
        $this->admin();
        $this->app->csrf->verify($request);
    ...
    public function projectAccess(Request $request): Response
    {
        $this->admin();
        $this->app->csrf->verify($request);
        $projectId = (int) $request->param('id');
    ...
    public function removeProjectMembership(Request $request): Response
    {
        $this->admin();
        $this->app->csrf->verify($request);
        $projectId = (int) $request->param('id');
    ...
    public function removeProjectAccess(Request $request): Response
    {
        $this->admin();
        $this->app->csrf->verify($request);
        $projectId = (int) $request->param('id');
    ...
    private function createUser(Request $request): int
    {
        ...
        $statement = $this->app->pdo()->prepare(
            'INSERT INTO users (role_id, username, email, password_hash, locale) SELECT id, :username, :email, :password, :locale FROM roles WHERE slug = :role'
        );
    ...
    private function updateUser(int $id, Request $request): void
    {
        ...
        $role = (string) $request->input('role', 'user');
        if (!in_array($status, ['active', 'blocked', 'pending'], true) || !in_array($role, ['admin', 'user'], true)) {
            throw new HttpException(422, 'Invalid user status or role.');
        }
        if ($id === $this->app->auth()->id() && ($status !== 'active' || $role !== 'admin')) {
            throw new HttpException(409, 'You cannot remove your own active administrator access.');
        }
        $password = (string) $request->input('password', '');
        if ($password !== '' && strlen($password) < 12) {
            throw new HttpException(422, 'Password needs at least 12 characters.');
        }
        ...
        $sql .= ' WHERE id = :id';
        $this->app->pdo()->prepare($sql)->execute($params);
    }
    ...
    private function createProject(Request $request): int
    {
        ...
        if ($name === '' || preg_match('/^[a-z0-9][a-z0-9-]{1,99}$/', $slug) !== 1) {
            throw new HttpException(422, 'Invalid project name or slug.');
        }
    ...
    private function updateProject(int $id, Request $request): void
    {
        ...
        if (!in_array($status, ['active', 'suspended'], true)) {
            throw new HttpException(422, 'Invalid project status.');
        }
    ...
    private function createProxmox(Request $request): int
    {
        ...
        try {
            $client = new ProxmoxClient(
                $config['hostname'], $config['port'], $config['realm'], $config['token_id'],
                $config['token_secret'], $config['verify_ssl'],
            );
            ...
            throw new HttpException(422, ProxmoxFailureMessage::describe(
                $exception, $config['hostname'], $config['port'], [$config['token_secret']],
            ), $details);
        ...
        $statement = $this->app->pdo()->prepare(
            'INSERT INTO proxmox_connections (name, hostname, port, realm, api_token_id, api_token_secret_encrypted, verify_ssl, cluster_name, last_checked_at, created_by)
             VALUES (:name, :host, :port, :realm, :token, :secret, :verify, :cluster, CURRENT_TIMESTAMP, :user)'
        );
        $statement->execute([
            'name' => $config['name'], 'host' => $config['hostname'], 'port' => $config['port'],
            'realm' => $config['realm'], 'token' => $config['token_id'],
            'secret' => $this->app->crypto()->encrypt($config['token_secret']),
            'verify' => (int) $config['verify_ssl'], 'cluster' => $clusterName,
    ...
    private function updateProxmox(int $id, Request $request): void
    {
        ...
        if (!in_array($status, ['active', 'disabled'], true)) {
            throw new HttpException(422, 'Invalid connection status.');
        }
    ...
    private function createPlan(Request $request): int
    {
        ...
        if ($name === '' || preg_match('/^[a-z0-9][a-z0-9-]{1,99}$/', $slug) !== 1) {
            throw new HttpException(422, 'Invalid plan name or slug.');
        }
    ...
    private function upsertQuota(Request $request): int
    {
        $subject = (string) $request->input('subject_type');
        if (!in_array($subject, ['project', 'user'], true)) {
            throw new HttpException(422, 'Invalid quota subject.');
        }
        $subjectId = $this->positiveInt($request->input('subject_id'), 1, PHP_INT_MAX);
        $column = $subject === 'project' ? 'project_id' : 'user_id';
        $values = [
            'max_vms' => $this->positiveInt($request->input('max_vms'), 0, 100000),
            'max_vcpu' => $this->positiveInt($request->input('max_vcpu'), 0, 1000000),
            'max_ram_mb' => $this->positiveInt($request->input('max_ram_mb'), 0, PHP_INT_MAX),
            'max_storage_gb' => $this->positiveInt($request->input('max_storage_gb'), 0, PHP_INT_MAX),
            'max_snapshots' => $this->positiveInt($request->input('max_snapshots'), 0, 1000000),
        ];
        $maxIps = $request->input('max_ip_addresses');
        $values['max_ip_addresses'] = $maxIps === null || $maxIps === '' ? null : $this->positiveInt($maxIps, 0, 1000000);
        $sql = "INSERT INTO quotas ({$column}, max_vms, max_vcpu, max_ram_mb, max_storage_gb, max_snapshots, max_ip_addresses)
                VALUES (:subject, :max_vms, :max_vcpu, :max_ram_mb, :max_storage_gb, :max_snapshots, :max_ip_addresses)
                ON DUPLICATE KEY UPDATE max_vms=VALUES(max_vms), max_vcpu=VALUES(max_vcpu), max_ram_mb=VALUES(max_ram_mb),
                  max_storage_gb=VALUES(max_storage_gb), max_snapshots=VALUES(max_snapshots), max_ip_addresses=VALUES(max_ip_addresses)";
    ...
    private function createNetwork(Request $request): int
    {
        return (new Database($this->app->config))->transaction(function (PDO $pdo) use ($request): int {
            ...
            $vlan = $vlan === null || $vlan === '' ? null : $this->positiveInt($vlan, 1, 4094);
            if ($name === '' || preg_match('/^[A-Za-z0-9_.-]{1,32}$/', $bridge) !== 1 || !$this->validIpv4Range($subnet, $start, $end, $gateway)) {
                throw new HttpException(422, 'Invalid network or IPv4 range.');
            }
            ...
            foreach (is_array($nodes) ? $nodes : [] as $node) {
                if (!is_array($node) || empty($node['node'])) continue;
                if (($node['status'] ?? null) !== 'online') continue;
                ...
                foreach (is_array($interfaces) ? $interfaces : [] as $interface) {
                    if (is_array($interface) && ($interface['iface'] ?? null) === $bridge && in_array(($interface['type'] ?? null), ['bridge', 'OVSBridge'], true)) {
                        $matchedNodes++;
                        $hostAddress = trim((string) ($interface['address'] ?? ''));
                        if ($hostAddress === '' && is_string($interface['cidr'] ?? null)) {
                            $hostAddress = explode('/', $interface['cidr'], 2)[0];
                        }
                        if (filter_var($hostAddress, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4) !== false) {
                            $reservedAddresses[$hostAddress] = true;
                        }
                        break;
                    }
                }
            }
            if ($eligibleNodes === 0 || $matchedNodes !== $eligibleNodes) {
                throw new HttpException(422, $nodeName === '' ? 'A cluster-wide bridge must exist on every active Proxmox node.' : 'The bridge was not found on the selected Proxmox node.');
            }
            $dns = array_values(array_filter(array_map('trim', explode(',', (string) $request->input('dns_servers', '')))));
            if (count($dns) > 3 || array_filter($dns, static fn (string $address): bool => filter_var($address, FILTER_VALIDATE_IP) === false) !== []) {
                throw new HttpException(422, 'DNS servers must be a comma-separated list of at most three IP addresses.');
            }
            ...
            if ($last - $first > 65535) {
                throw new HttpException(422, 'IP range may contain at most 65,536 addresses.');
            }
            $inserted = 0;
            ...
            if ($inserted === 0) {
                throw new HttpException(422, 'The IP range contains no assignable addresses after excluding the gateway and Proxmox node addresses.');
            }
            return $id;
        });
    }
    ...
    private function createTemplate(Request $request): int
    {
        ...
        if ($node === '') {
            throw new HttpException(422, 'Node name is required.');
        }
        $config = (new ProxmoxClientFactory($this->app->pdo(), $this->app->crypto()))
            ->forConnection($connectionId)
            ->get('/nodes/' . rawurlencode($node) . '/qemu/' . $vmid . '/config');
        if (!is_array($config) || (int) ($config['template'] ?? 0) !== 1) {
            throw new HttpException(422, 'The selected Proxmox VM is not a template.');
        }
        if (!is_string($config['scsi0'] ?? null) || preg_match('/(?:^|,)size=\d+(?:\.\d+)?[KMGT](?:,|$)/i', (string) $config['scsi0']) !== 1) {
            throw new HttpException(422, 'The template must expose its primary disk as scsi0 with a readable size.');
        }
        $hasCloudInit = false;
        ...
        if (!$hasCloudInit) {
            throw new HttpException(422, 'The template does not contain a cloud-init drive.');
        }
        ...
        $statement->execute([
            ...
            'vmid' => $vmid,
            'name' => trim((string) $request->input('name')),
            'os' => mb_substr((string) $request->input('operating_system', ''), 0, 100),
            'description' => mb_substr((string) $request->input('description', ''), 0, 5000),
        ]);
        return (int) $this->app->pdo()->lastInsertId();
    }
    ...
    private function createStorage(Request $request): int
    {
        ...
        $nodeName = trim((string) $request->input('node_name', ''));
        if ($storageName === '') {
            throw new HttpException(422, 'Storage name is required.');
        }
        ...
        if (!is_array($config)) {
            throw new HttpException(422, 'Storage was not found in Proxmox.');
        }
        $contentTypes = array_filter(array_map('trim', explode(',', (string) ($config['content'] ?? ''))));
        if (!in_array('images', $contentTypes, true)) {
            throw new HttpException(422, 'Storage is not configured for VM disk images.');
        }
        $nodes = $client->get('/nodes');
        $eligible = 0;
        $available = 0;
        foreach (is_array($nodes) ? $nodes : [] as $node) {
            if (!is_array($node) || empty($node['node']) || ($node['status'] ?? null) !== 'online') {
                continue;
            }
            ...
            try {
                ...
                if (is_array($status) && (int) ($status['active'] ?? 0) === 1 && (int) ($status['enabled'] ?? 1) === 1) {
                    $available++;
                }
            } catch (\Throwable) {
            }
        }
        if ($eligible === 0 || $available !== $eligible) {
            throw new HttpException(422, $nodeName === '' ? 'Cluster-wide storage must be active on every online node.' : 'Storage is not active on the selected node.');
        }
        ...
        $statement->execute([
            'connection' => $connectionId,
            'node' => $nodeName === '' ? null : $nodeName,
            'name' => $storageName,
            'content' => mb_substr(implode(',', $contentTypes), 0, 255),
        ]);
        return (int) $this->app->pdo()->lastInsertId();
    }
    ...
    private function upsertSetting(Request $request): int
    {
        ...
        if ($key === 'portal.default_locale' && !in_array($value, ['pl', 'en'], true)) {
            throw new HttpException(422, 'portal.default_locale must be pl or en.');
        }
        $statement = $this->app->pdo()->prepare(
    ...
    {
        if (!in_array($table, ['networks', 'vm_templates', 'storages'], true)) {
            throw new \LogicException('Unsafe table name.');
        }
        $this->app->pdo()->prepare("UPDATE {$table} SET enabled = :enabled WHERE id = :id")->execute([
            'enabled' => (int) filter_var($request->input('enabled', true), FILTER_VALIDATE_BOOL), 'id' => $id,
        ]);
    }
    ...
    {
        if (preg_match('#^([^/]+)/(\d{1,2})$#', $subnet, $match) !== 1 || filter_var($match[1], FILTER_VALIDATE_IP, FILTER_FLAG_IPV4) === false) {
            return false;
        }
        $prefix = (int) $match[2];
        if ($prefix < 8 || $prefix > 30 || filter_var($start, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4) === false || filter_var($end, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4) === false || ($gateway !== '' && filter_var($gateway, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4) === false)) {
            return false;
        }
    ...
    {
        return $this->app->pdo()->query(
            "SELECT id, name, hostname, port, status FROM proxmox_connections WHERE status <> 'disabled' ORDER BY name"
        )->fetchAll();
    }
    ...
    {
        return ProxmoxFailureMessage::asHttpException(
            $exception, (string) $connection['hostname'], (int) $connection['port'], $operation,
        );
    }
