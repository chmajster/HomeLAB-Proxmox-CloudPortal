...
final class JobRepository
{
    ...
    public function enqueue(string $type, ?int $userId, ?int $projectId, ?int $connectionId, array $payload, ?string $reservationKey = null, ?int $vmId = null, int $maxAttempts = 3): string
    {
        ...
        $statement = $this->pdo->prepare(
            'INSERT INTO jobs (public_id, type, user_id, project_id, virtual_machine_id, connection_id, reservation_key, payload, max_attempts)
             VALUES (:public_id, :type, :user, :project, :vm, :connection, :reservation, :payload, :max_attempts)'
        );
        $statement->execute([
            'public_id' => $publicId,
            'type' => $type,
    ...
    public function claimNext(): ?array
    {
        ...
        try {
            ...
            if ((int) $job['attempts'] >= (int) $job['max_attempts']) {
                $this->pdo->prepare("UPDATE jobs SET status='dead_letter', dead_letter_at=CURRENT_TIMESTAMP, finished_at=CURRENT_TIMESTAMP WHERE id=:id")
                    ->execute(['id' => $job['id']]);
                $this->pdo->commit();
                return null;
            }
            $this->pdo->prepare(
                "UPDATE jobs SET status='running', started_at=CURRENT_TIMESTAMP, finished_at=NULL, attempts=attempts+1 WHERE id=:id"
            )->execute(['id' => $job['id']]);
            $this->pdo->commit();
            $job['status'] = 'running';
    ...
    {
        $this->pdo->prepare(
            "UPDATE jobs SET status='completed', result=:result, error_message=NULL, finished_at=CURRENT_TIMESTAMP, dead_letter_at=NULL WHERE id=:id"
        )->execute([
    ...
    {
        $statement = $this->pdo->prepare('SELECT attempts, max_attempts FROM jobs WHERE id=:id FOR UPDATE');
        ...
        if ($attempts < $maxAttempts) {
            ...
            $this->pdo->prepare(
                "UPDATE jobs SET status='queued', error_message=:message, available_at=:available, finished_at=NULL WHERE id=:id"
            )->execute([
                'message' => $safeMessage,
        ...
        $this->pdo->prepare(
            "UPDATE jobs SET status='dead_letter', error_message=:message, finished_at=CURRENT_TIMESTAMP, dead_letter_at=CURRENT_TIMESTAMP WHERE id=:id"
    ...
    {
        $statement = $this->pdo->prepare(
            "UPDATE jobs SET status='queued', attempts=0, error_message=NULL, result=NULL, proxmox_upid=NULL,
                    available_at=CURRENT_TIMESTAMP, started_at=NULL, finished_at=NULL, dead_letter_at=NULL
             WHERE public_id=:id AND status IN ('failed','dead_letter')"
        );
    ...
    public function find(string $publicId): ?array
    {
        ...
        $job['payload'] = json_decode((string) $job['payload'], true, 64, JSON_THROW_ON_ERROR);
        if (is_string($job['result'] ?? null)) {
            $job['result'] = json_decode((string) $job['result'], true, 64);
        }
        return $job;
    }
    ...
    public function metrics(): array
    {
        ...
        foreach ($rows as $row) {
            if (isset($result[(string) $row['status']])) {
    ...
    public function retainedFailedCreates(): array
    {
        $statement = $this->pdo->query(
            "SELECT j.* FROM jobs j JOIN quota_reservations qr ON qr.reservation_key=j.reservation_key
             WHERE j.status IN ('failed','dead_letter') AND j.type='vm.create' AND qr.retain_until_reconciled=1 ORDER BY j.id LIMIT 100"
        );
        $jobs = $statement->fetchAll();
        foreach ($jobs as &$job) {
            $job['payload'] = json_decode((string) $job['payload'], true, 64, JSON_THROW_ON_ERROR);
