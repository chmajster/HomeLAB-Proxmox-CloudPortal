...
final class AuditLogger
{
    ...
    public function log(?int $userId, string $ip, string $action, string $result, ?string $resourceType = null, string|int|null $resourceId = null, array $metadata = []): void
    {
        ...
        $statement = $this->pdo->prepare(
            'INSERT INTO audit_logs
             (user_id,project_id,virtual_machine_id,job_id,ip_address,action,resource_type,resource_id,proxmox_upid,result,metadata)
             VALUES (:user_id,:project_id,:vm_id,:job_id,:ip,:action,:resource_type,:resource_id,:upid,:result,:metadata)'
        );
        $statement->execute([
            'user_id' => $userId,
    ...
    /**
     * @param array<string,mixed> $metadata
     * @return array{project_id:?int,virtual_machine_id:?int,job_id:?int,proxmox_upid:?string}
     */
    private function context(?string $resourceType, string|int|null $resourceId, array $metadata): array
    {
        ...
        try {
            ...
            if ($jobReference !== null && $jobReference !== '') {
                $statement = $this->pdo->prepare('SELECT id,project_id,virtual_machine_id,proxmox_upid FROM jobs WHERE public_id=:ref LIMIT 1');
                $statement->execute(['ref' => $jobReference]);
                $row = $statement->fetch();
                if (is_array($row)) $this->mergeJob($context, $row);
            } elseif ($context['job_id'] !== null) {
                $statement = $this->pdo->prepare('SELECT id,project_id,virtual_machine_id,proxmox_upid FROM jobs WHERE id=:id LIMIT 1');
    ...
    /** @param array{project_id:?int,virtual_machine_id:?int,job_id:?int,proxmox_upid:?string} $context @param array<string,mixed> $row */
